...
void gl_retained_camera::set_uniform_vals() const {
	...
	shaderStore.set_uniform_value( SHADER_PROJECTION_UNIFORM_NAME, m_projection.get_raw_transformation() );
	shaderStore.set_uniform_value( SHADER_VIEW_UNIFORM_NAME, m_view.get_raw_transformation() );
}
...
void gl_retained_camera::check_store() const {
	...
	if( !shaderStore.has_uniform( SHADER_PROJECTION_UNIFORM_NAME ) )
		shaderStore.add_uniform( SHADER_PROJECTION_UNIFORM_NAME, m_projection.get_raw_transformation() );

	if( !shaderStore.has_uniform( SHADER_VIEW_UNIFORM_NAME ) )
		shaderStore.add_uniform( SHADER_VIEW_UNIFORM_NAME, m_view.get_raw_transformation() );
}
